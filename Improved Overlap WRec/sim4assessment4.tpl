...
DATA_SECTION
  ...
  !!fpop=1;
  init_int lpop
  init_matrix Mix(fpop,lpop,fpop,lpop)
  init_int lyear                                    //Number of years
  init_int fage
  init_int lage                                     //Number of ages
  //added on Jan30,2017
  int totPopAge
  !!totPopAge=(lpop-fpop+1)*(lage-fage+1);
  ...
  init_3darray rc_catch_popcomp1(fyear,lyear,fpop,lpop,fage,lage)      // catch at pop and age
  init_3darray rc_catch_popcomp2(fyear,lyear,fpop,lpop,fage,lage)      // catch at pop and age
  init_3darray rc_catch_popcomp3(fyear,lyear,fpop,lpop,fage,lage)      // catch at pop and age
  init_3darray rc_catch_popcomp4(fyear,lyear,fpop,lpop,fage,lage)      // catch at pop and age
  ...
  matrix rc_cps1(fyear,lyear,1,totPopAge)          //real age pop composition from the recreational catch
  matrix rc_cps2(fyear,lyear,1,totPopAge)
  matrix rc_cps3(fyear,lyear,1,totPopAge)
  matrix rc_cps4(fyear,lyear,1,totPopAge)
  matrix obsResampleCatch1(fyear,lyear,1,totPopAge)  //observed catch based on input actual age composition and effective sample size
  matrix obsResampleCatch2(fyear,lyear,1,totPopAge)
  matrix obsResampleCatch3(fyear,lyear,1,totPopAge)
  matrix obsResampleCatch4(fyear,lyear,1,totPopAge)
  matrix new_rc_cps1(fyear,lyear,1,totPopAge)    //observed age pop composition from resampling readin catch
  matrix new_rc_cps2(fyear,lyear,1,totPopAge)
  matrix new_rc_cps3(fyear,lyear,1,totPopAge)
  matrix new_rc_cps4(fyear,lyear,1,totPopAge)
...
PARAMETER_SECTION
  ...
  init_bounded_number log_initial1(9.,19.,1)                 //Numbers of age-2 and older fish in the first year
  init_bounded_number log_initial2(9.,19.,1) 
  init_bounded_number log_initial3(9.,19.,1) 
  init_bounded_number log_initial4(9.,19.,1) 
  init_bounded_dev_vector initial_devs1(fage+1,lage,-7.,7.,2)
  init_bounded_dev_vector initial_devs2(fage+1,lage,-7.,7.,2)
  init_bounded_dev_vector initial_devs3(fage+1,lage,-7.,7.,2)
  init_bounded_dev_vector initial_devs4(fage+1,lage,-7.,7.,2)
  ...
  init_bounded_dev_vector recruit_devs1(fyear,lyear,-5.,5.,2)  //Annual recruitment deviations
  init_bounded_dev_vector recruit_devs2(fyear,lyear,-5.,5.,2)
  init_bounded_dev_vector recruit_devs3(fyear,lyear,-5.,5.,2)
  init_bounded_dev_vector recruit_devs4(fyear,lyear,-5.,5.,2)
  init_bounded_dev_vector log_effort_devs1(fyear,lyear,-20.,20.,3)
  init_bounded_dev_vector log_effort_devs2(fyear,lyear,-20.,20.,3)
  init_bounded_dev_vector log_effort_devs3(fyear,lyear,-20.,20.,3)
  init_bounded_dev_vector log_effort_devs4(fyear,lyear,-20.,20.,3)
  init_bounded_number log_sigma_rc(-5.,5.,3)                 //log sd for rec catch
  ...
  vector f1(fyear,lyear)              //Instantaneous fully selected fishing mortality matrix
  vector f2(fyear,lyear)
  vector f3(fyear,lyear)
  vector f4(fyear,lyear)
  ...
  vector zerovec(fyear,lyear)

  matrix n_rc1Pop1(fyear,lyear,fage,lage)	       //Predicted recreational catch at age
  matrix n_rc1Pop2(fyear,lyear,fage,lage)
  matrix n_rc1Pop3(fyear,lyear,fage,lage)
  matrix n_rc1Pop4(fyear,lyear,fage,lage)

  matrix n_rc2Pop1(fyear,lyear,fage,lage)	       //Predicted recreational catch at age
  matrix n_rc2Pop2(fyear,lyear,fage,lage)
  matrix n_rc2Pop3(fyear,lyear,fage,lage)
  matrix n_rc2Pop4(fyear,lyear,fage,lage)

  matrix n_rc3Pop1(fyear,lyear,fage,lage)	       //Predicted recreational catch at age
  matrix n_rc3Pop2(fyear,lyear,fage,lage)
  matrix n_rc3Pop3(fyear,lyear,fage,lage)
  matrix n_rc3Pop4(fyear,lyear,fage,lage)

  matrix n_rc4Pop1(fyear,lyear,fage,lage)	       //Predicted recreational catch at age
  matrix n_rc4Pop2(fyear,lyear,fage,lage)
  matrix n_rc4Pop3(fyear,lyear,fage,lage)
  matrix n_rc4Pop4(fyear,lyear,fage,lage)


  3darray n_rc_pop_cps1(fyear,lyear,fpop,lpop,fage,lage)
  3darray n_rc_pop_cps2(fyear,lyear,fpop,lpop,fage,lage)
  3darray n_rc_pop_cps3(fyear,lyear,fpop,lpop,fage,lage)
  3darray n_rc_pop_cps4(fyear,lyear,fpop,lpop,fage,lage)

  matrix n_rc_cps1(fyear,lyear,1,totPopAge)    //age composition from resampling readin catch
  matrix n_rc_cps2(fyear,lyear,1,totPopAge)
  matrix n_rc_cps3(fyear,lyear,1,totPopAge)
  matrix n_rc_cps4(fyear,lyear,1,totPopAge)
...
PRELIMINARY_CALCS_SECTION 
  ...
  {   
   rc_cch1(i)=sum(rc_catch_popcomp1(i));                     //Total catch in a year
   rc_cch2(i)=sum(rc_catch_popcomp2(i));  
   rc_cch3(i)=sum(rc_catch_popcomp3(i));  
   rc_cch4(i)=sum(rc_catch_popcomp4(i));  
   //add observation error for read in catch
   new_rc_cch1(i)=rc_cch1(i)*mfexp((stdObsCatch*randn(rnd))-(0.5*square(stdObsCatch))); 
   ...
   new_rc_cch4(i)=rc_cch4(i)*mfexp((stdObsCatch*randn(rnd))-(0.5*square(stdObsCatch))); 
  }
  ...
  for(i=fyear;i<=lyear;i++)
  {
   for(k=fpop; k<=lpop; k++) 
     ...
     for(j=fage;j<=lage;j++)
     {
      ...
      int w=j+(k-1)*numAge;         //vector correspond to multinomial
      w=w-(fage-1);          //be caution, trick here to make vector index start from 1
      rc_cps1(i,w)=rc_catch_popcomp1(i,k,j)/rc_cch1(i);     //age comp by division
      rc_cps2(i,w)=rc_catch_popcomp2(i,k,j)/rc_cch2(i); 
      rc_cps3(i,w)=rc_catch_popcomp3(i,k,j)/rc_cch3(i);
      rc_cps4(i,w)=rc_catch_popcomp4(i,k,j)/rc_cch4(i);
    } 
   }
  }

  /*
  cout<<"input  rec catch at pop and age in fyear"<<endl;   
  cout<<rc_catch_popcomp1(fyear)<<endl;
  cout<<"Real total rec catch1 in fyear"<<endl;   
  cout<<rc_cch1(fyear)<<endl;
  exit(10);
 */

  /*
  cout<<"Real total rec catch1"<<endl;   
  cout<<rc_cch1<<endl;
  cout<<"observed total catch1"<<endl;
  cout<<new_rc_cch1<<endl;


  cout<<"Real total rec catch2"<<endl;   
  cout<<rc_cch2<<endl;
  cout<<"observed total catch2"<<endl;
  cout<<new_rc_cch2<<endl;

  cout<<"Real total rec catch3"<<endl;   
  cout<<rc_cch3<<endl;
  cout<<"observed total catch3"<<endl;
  cout<<new_rc_cch3<<endl;

  cout<<"Real total rec catch4"<<endl;   
  cout<<rc_cch4<<endl;
  cout<<"observed total catch4"<<endl;
  cout<<new_rc_cch4<<endl;
  */

  //cout<<"Age composition"<<endl;
  //cout<<rc_cps<<endl;
  //cout<<"Catch_pop_comp1"<<endl;
  //cout<<rc_pop_cps1<<endl;
  //cout<<"Catch_pop_comp2"<<endl;
  //cout<<rc_pop_cps2<<endl;
  //cout<<"Catch_pop_comp3"<<endl;
  //cout<<rc_pop_cps3<<endl;
  //cout<<"Catch_pop_comp4"<<endl;
  //cout<<rc_pop_cps4<<endl;
  //exit(0);
  ...
  for(i=fyear;i<=lyear;i++){
    ...
    obsResampleCatch1(i)= CountFreq(fish1,fage,lage,fpop,lpop);  //total is effSampleSize
    obsResampleCatch2(i)= CountFreq(fish2,fage,lage,fpop,lpop);
    obsResampleCatch3(i)= CountFreq(fish3,fage,lage,fpop,lpop);
    obsResampleCatch4(i)= CountFreq(fish4,fage,lage,fpop,lpop);
    ...
    /*
   cout<<"Year"<<i<<"real and obs cps1"<<endl;
   cout<<rc_cps1(i)<<endl;
   cout<<new_rc_cps1(i)<<endl;
   cout<<"Year"<<i<<" real and obs cps2"<<endl;
   cout<<rc_cps2(i)<<endl;
   cout<<new_rc_cps2(i)<<endl;
   cout<<"Year"<<i<<" real and obs cps3"<<endl;
   cout<<rc_cps3(i)<<endl;
   cout<<new_rc_cps3(i)<<endl;
   cout<<"Year"<<i<<" real and obs cps4"<<endl;
   cout<<rc_cps4(i)<<endl;
   cout<<new_rc_cps4(i)<<endl;
   
 
   cout<<"Year"<<i<<" real cps1"<<endl;
   cout<<sum(rc_cps1(i))<<endl;
   cout<<sum(new_rc_cps1(i))<<endl;
   cout<<"Year"<<i<<" real cps2"<<endl;
   cout<<sum(rc_cps2(i))<<endl;
   cout<<sum(new_rc_cps2(i))<<endl;
   cout<<"Year"<<i<<" real cps3"<<endl;
   cout<<sum(rc_cps3(i))<<endl;
   cout<<sum(new_rc_cps3(i))<<endl;
   cout<<"Year"<<i<<" real cps4"<<endl;
   cout<<sum(rc_cps4(i))<<endl;
   cout<<sum(new_rc_cps4(i))<<endl; 
   */
  }
  //exit(10);
  ...
  //cout<<"New_Catch_pop_comp1"<<endl;
  //cout<<new_rc_pop_cps1<<endl;
  //cout<<"New_Catch_pop_comp2"<<endl;
  //cout<<new_rc_pop_cps2<<endl;
  //cout<<"New_Catch_pop_comp3"<<endl;
  //cout<<new_rc_pop_cps3<<endl;
  //cout<<"New_Catch_pop_comp4"<<endl;
  //cout<<new_rc_pop_cps4<<endl;
...
FUNCTION dvector CountFreq(const ivector& sample,int fage,int lage, int fpop, int lpop)
  //int numAge=lage-fage+1;
  //ivector uniqAge(fage,lage);

  int numAge=lage-fage+1;
  int numPop=lpop-fpop+1;
  int numPopAge=numAge*numPop;
  ivector uniqCatg(1,numPopAge);
  uniqCatg.fill_seqadd(1,1);
 
  dvector freq(1,numPopAge); 
  freq.initialize();    

  //count the number of ages which at the same age
  for(int i=sample.indexmin();i<=sample.indexmax();i++){
   for(int k=fpop;k<=lpop;k++){
    for(int j=fage;j<=lage;j++){
      //int numAge=lage-fage+1;       
      int w=j+(k-1)*numAge;         //vector correspond to multinomial
      w=w-(fage-1);                 //be caution, trick here to make vector index start from 1
      if(sample(i)==uniqCatg(w)){ // found match
       freq(w)+=1;
       break;
      }
    }
...
FUNCTION get_mortality
  ...
  for(i=fyear;i<=lyear;i++)
   {
     f1(i)=qrc*rc_eff1(i)*effort_devs1(i);
     f2(i)=qrc*rc_eff2(i)*effort_devs2(i);
     f3(i)=qrc*rc_eff3(i)*effort_devs3(i);
     f4(i)=qrc*rc_eff4(i)*effort_devs4(i);
    for(j=fage;j<=lage;j++)
     {
     F1(i,j)=f1(i)*rc_s(i,j);
     F2(i,j)=f2(i)*rc_s(i,j);
     F3(i,j)=f3(i)*rc_s(i,j);
     F4(i,j)=f4(i)*rc_s(i,j);
     }
   }
  Z1= M + F1;
  Z2= M + F2;
...
FUNCTION get_catch
  ...
  n_rc1Pop1=elem_prod(elem_div(F1,Z1),elem_prod(1.0-S1,n1*Mix(1,1)));	//Baranov catch equation for recreational harvest
  n_rc2Pop1=elem_prod(elem_div(F2,Z2),elem_prod(1.0-S2,n1*Mix(1,2)));
  n_rc3Pop1=elem_prod(elem_div(F3,Z3),elem_prod(1.0-S3,n1*Mix(1,3)));
  n_rc4Pop1=elem_prod(elem_div(F4,Z4),elem_prod(1.0-S4,n1*Mix(1,4)));


  n_rc1Pop2=elem_prod(elem_div(F1,Z1),elem_prod(1.0-S1,n2*Mix(2,1)));	//Baranov catch equation for recreational harvest
  n_rc2Pop2=elem_prod(elem_div(F2,Z2),elem_prod(1.0-S2,n2*Mix(2,2)));
  n_rc3Pop2=elem_prod(elem_div(F3,Z3),elem_prod(1.0-S3,n2*Mix(2,3)));
  n_rc4Pop2=elem_prod(elem_div(F4,Z4),elem_prod(1.0-S4,n2*Mix(2,4)));

  n_rc1Pop3=elem_prod(elem_div(F1,Z1),elem_prod(1.0-S1,n3*Mix(3,1)));	//Baranov catch equation for recreational harvest
  n_rc2Pop3=elem_prod(elem_div(F2,Z2),elem_prod(1.0-S2,n3*Mix(3,2)));
  n_rc3Pop3=elem_prod(elem_div(F3,Z3),elem_prod(1.0-S3,n3*Mix(3,3)));
  n_rc4Pop3=elem_prod(elem_div(F4,Z4),elem_prod(1.0-S4,n3*Mix(3,4)));

  n_rc1Pop4=elem_prod(elem_div(F1,Z1),elem_prod(1.0-S1,n4*Mix(4,1)));	//Baranov catch equation for recreational harvest
  n_rc2Pop4=elem_prod(elem_div(F2,Z2),elem_prod(1.0-S2,n4*Mix(4,2)));
  n_rc3Pop4=elem_prod(elem_div(F3,Z3),elem_prod(1.0-S3,n4*Mix(4,3)));
  n_rc4Pop4=elem_prod(elem_div(F4,Z4),elem_prod(1.0-S4,n4*Mix(4,4)));



  for (i=fyear; i<=lyear; i++)
   {
   N_rc1(i)=sum(n_rc1Pop1(i))+sum(n_rc1Pop2(i))+sum(n_rc1Pop3(i))+sum(n_rc1Pop4(i));
   N_rc2(i)=sum(n_rc2Pop1(i))+sum(n_rc2Pop2(i))+sum(n_rc2Pop3(i))+sum(n_rc2Pop4(i));
   N_rc3(i)=sum(n_rc3Pop1(i))+sum(n_rc3Pop2(i))+sum(n_rc3Pop3(i))+sum(n_rc3Pop4(i));
   N_rc4(i)=sum(n_rc4Pop1(i))+sum(n_rc4Pop2(i))+sum(n_rc4Pop3(i))+sum(n_rc4Pop4(i)); 
   }


   //Estimated pop and age composition of recreational catch
  for (i=fyear; i<=lyear; i++)
  {
   n_rc_pop_cps1(i,1)=n_rc1Pop1(i)/N_rc1(i);
   n_rc_pop_cps1(i,2)=n_rc1Pop2(i)/N_rc1(i);
   n_rc_pop_cps1(i,3)=n_rc1Pop3(i)/N_rc1(i);
   n_rc_pop_cps1(i,4)=n_rc1Pop4(i)/N_rc1(i);
   n_rc_pop_cps2(i,1)=n_rc2Pop1(i)/N_rc2(i);
   n_rc_pop_cps2(i,2)=n_rc2Pop2(i)/N_rc2(i);
   n_rc_pop_cps2(i,3)=n_rc2Pop3(i)/N_rc2(i);
   n_rc_pop_cps2(i,4)=n_rc2Pop4(i)/N_rc2(i);
   n_rc_pop_cps3(i,1)=n_rc3Pop1(i)/N_rc3(i);
   n_rc_pop_cps3(i,2)=n_rc3Pop2(i)/N_rc3(i);
   n_rc_pop_cps3(i,3)=n_rc3Pop3(i)/N_rc3(i);
   n_rc_pop_cps3(i,4)=n_rc3Pop4(i)/N_rc3(i);
   n_rc_pop_cps4(i,1)=n_rc4Pop1(i)/N_rc4(i);
   n_rc_pop_cps4(i,2)=n_rc4Pop2(i)/N_rc4(i);
   n_rc_pop_cps4(i,3)=n_rc4Pop3(i)/N_rc4(i);
   n_rc_pop_cps4(i,4)=n_rc4Pop4(i)/N_rc4(i);
   for(int k=fpop;k<=lpop;k++){
    for(int j=fage;j<=lage;j++){
      int numAge=lage-fage+1;       
      int w=j+(k-1)*numAge;         //vector correspond to multinomial
      w=w-(fage-1);                 //be caution, trick here to make vector index start from
      n_rc_cps1(i,w)=n_rc_pop_cps1(i,k,j);
      n_rc_cps2(i,w)=n_rc_pop_cps2(i,k,j);
      n_rc_cps3(i,w)=n_rc_pop_cps3(i,k,j);
      n_rc_cps4(i,w)=n_rc_pop_cps4(i,k,j);
    }
   }
 }
  ...
  //cout<<"Est_Catch_pop_comp1"<<endl;
  //cout<<n_rc_pop_cps1<<endl;
  //cout<<"Est_Catch_pop_comp2"<<endl;
  //cout<<n_rc_pop_cps2<<endl;
  //cout<<"Est_Catch_pop_comp3"<<endl;
  //cout<<n_rc_pop_cps3<<endl;
  //cout<<"Est_Catch_pop_comp4"<<endl;
  //cout<<n_rc_pop_cps4<<endl;
...
FUNCTION get_objective
  ...
  L2_1 = -sum(double(effSampleSize)*elem_prod(new_rc_cps1,log(0.0000001+n_rc_cps1)));     //age comp for recreational fishery, old way
  L2_2 = -sum(double(effSampleSize)*elem_prod(new_rc_cps2,log(0.0000001+n_rc_cps2)));
  L2_3 = -sum(double(effSampleSize)*elem_prod(new_rc_cps3,log(0.0000001+n_rc_cps3)));
  L2_4 = -sum(double(effSampleSize)*elem_prod(new_rc_cps4,log(0.0000001+n_rc_cps4)));
  ...
  L4_1 = nllNormal(recruit_devs1,zerovec,2.0);
  L4_2 = nllNormal(recruit_devs2,zerovec,2.0);
  L4_3 = nllNormal(recruit_devs3,zerovec,2.0);
  L4_4 = nllNormal(recruit_devs4,zerovec,2.0);




  negLL=L1_1 +L1_2 +L1_3 +L1_4 +L2_1 +L2_2 +L2_3 +L2_4 +L3_1 +L3_2 +L3_3 +L3_4+L4_1+L4_2+L4_3+L4_4;
    //negLL=L1_1 +L1_2 +L1_3 +L1_4 +L2_1 +L2_2 +L2_3 +L2_4 +L3_1 +L3_2 +L3_3 +L3_4+L5_1+L5_2+L5_3+L5_4;
...
REPORT_SECTION
  ...
  report<<Total_ssb4(lyear)<<endl;//terminal year ssb
  //report<<n1(lyear,fage)<<endl;
  //report<<n2(lyear,fage)<<endl;
  //report<<n3(lyear,fage)<<endl;
  //report<<n4(lyear,fage)<<endl;//terminal year recruit
  report<<column(n1,fage)(1,lyear)<<endl;
  report<<column(n2,fage)(1,lyear)<<endl;
  report<<column(n3,fage)(1,lyear)<<endl;
  report<<column(n4,fage)(1,lyear)<<endl;
  report<<f1<<endl;
  report<<f2<<endl;
  report<<f3<<endl;
  report<<f4<<endl;
  ...
  for (i=fyear; i<=lyear; i++)
  {
  report<<"Year"<<i<<" real cps1"<<endl;
  report<<rc_cps1(i)<<endl;
  report<<new_rc_cps1(i)<<endl;
  report<<n_rc_cps1(i)<<endl;
  report<<"Year"<<i<<" real cps2"<<endl;
  report<<rc_cps2(i)<<endl;
  report<<new_rc_cps2(i)<<endl;
  report<<n_rc_cps2(i)<<endl;
  report<<"Year"<<i<<" real cps3"<<endl;
  report<<rc_cps3(i)<<endl;
  report<<new_rc_cps3(i)<<endl;
  report<<n_rc_cps3(i)<<endl;
  report<<"Year"<<i<<" real cps4"<<endl;
  report<<rc_cps4(i)<<endl;
  report<<new_rc_cps4(i)<<endl;
  report<<n_rc_cps4(i)<<endl;
 }
 */
  /*
  report<<"Catch_pop_comp1"<<endl;
  report<<rc_cps1<<endl;
  report<<new_rc_cps1<<endl;
  report<<n_rc_cps1<<endl;
  report<<"Catch_pop_comp2"<<endl;
  report<<rc_cps2<<endl;
  report<<new_rc_cps2<<endl;
  report<<n_rc_cps2<<endl;
  report<<"Catch_pop_comp3"<<endl;
  report<<rc_cps3<<endl;
  report<<new_rc_cps3<<endl;
  report<<n_rc_cps3<<endl;
  report<<"Catch_pop_comp4"<<endl;
  report<<rc_cps4<<endl;
  report<<new_rc_cps4<<endl;
  report<<n_rc_cps4<<endl;
